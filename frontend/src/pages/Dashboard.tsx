...
export function Dashboard() {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!isDemoMode) {
        await api.post('/teleconsultations', newCase);
        fetchConsultations();
      } else {
  ...
  const filteredConsultations = consultations.filter(item => {
    const matchesSearch = item.patient_name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          item.diagnostic_hypothesis.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="min-h-screen bg-zinc-50/50 flex flex-col">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-zinc-200">
              <div className="hidden md:block text-right">
                <p className="text-sm font-semibold text-zinc-800">Corpo Clínico</p>
                <p className="text-xs text-zinc-400">Médico Ativo</p>
              </div>
              <button 
            ...
            {filteredConsultations.length === 0 ? (
              ...
              <div className="divide-y divide-zinc-100">
                {filteredConsultations.map((item) => (
                  <div 
                    key={item.id} 
                    onClick={() => { setSelectedCase(item); setIsDetailsModalOpen(true); }}
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-zinc-950/20 backdrop-blur-sm" onClick={() => !isSubmitting && setIsCreateModalOpen(false)}></div>
            ...
            {analyzingIA ? (
              ...
              <>
                ...
                <form onSubmit={handleCreateSubmit} className="space-y-4">
                  ...
                  >
                    <span>Confirmar e Analisar com IA</span>
                  </button>
                </form>
              </>
            )}
